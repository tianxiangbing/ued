...
(function(root, factory) {
	...
	return {
		init: function() {
			...
			/*var JsonList = (localStorage.qualification && JSON.parse(localStorage.qualification)) || {
				data: []
			};
			var List = JsonList.data || [];
			*/
			var JsonList ={};
			$.post('/entry/index/qualification',function(result){
				if(result.status){
					JsonList= result;
					bindList();
				}else{
			...
			$('.js-basic-infor').submit(function() {
				...
				JsonList.data = List;
				localStorage['qualification'] = JSON.stringify(JsonList);
				bindList();
				D[0].hide();
				return false;
			});
					...
					upload.init({
						...
						callback: function(result, file, name, target, i) {
							...
							//setTimeout(function() {
								...
								if (result.status) {
									$(_this).find('img').show();
									$(_this).children('img').attr("src", result.data.image_path+'/'+result.data.file_name);
									$(_this).nextAll('input').val(result.data.file_name);
								} else {
									$(target).addClass('error');
			...
			function bindInfo(index) {
				...
				if (obj) {
					$('[name="qualificationName"]').val(obj.qualificationName);
					$('[name="startTime"]').val(obj.startTime);
					$('[name="endTime"]').val(obj.endTime);
					$('[name="memo"]').val(obj.memo);
					$('[name="qualificationCertificate"]').val(obj.qualificationCertificate);
					$('.upload img').show().attr('src', obj.qualificationCertificate);
				}
			}
		}
	}
